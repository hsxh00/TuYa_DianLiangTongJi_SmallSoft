...
Page({
  ...
  sendDpon: async function (e) {
    const { device_id } = this.data

    const { success } = await deviceControl(device_id, 'switch_1', true)
  },
 
  sendDpoff: async function (e) {
    const { device_id } = this.data

    const { success } = await deviceControl(device_id, 'switch_1', false)
  },
